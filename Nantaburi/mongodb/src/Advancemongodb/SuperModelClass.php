...
use Nandev\Models\Group_type;
use Nandev\Models\Products;
use Illuminate\Support\Facades\Session;

class SuperModelClass  { //  defind Master Class 

    public $searchText='test';
    public $request_pagenumber=1;
    public $paginate_size=6;
    protected static $resultpaginate = array() ;  // output data array()

    private static $mainpipeline = array(
    ...
    public static function getTypeProducts($TypeId, $perpage, $request_page_number)
    {
         ...
         $pipline  = self::$mainpipeline ;
        // $finematch = array( '$match' => [ 'products.type_id' =>  $TypeId  ]  ) ; 
        $finematch = array( '$match' => [ 'products.type_id' =>  $TypeId  ]  ) ; 
        ...
        array_push($counter_pipeline, $option_count);
        array_push($pipline , $option_select);
    ...
    public static function getProductSearch ($searchText, $perpage, $request_page_number) {
        ...
        $findmatch = array ('$match'=> [ '$or'=> [ ['products.name'=> new Regex('^.*'. $searchText .'.*$', 'i')],
        ...
        $count_products=$mongodata->raw(function($collection) use ($counter_pipeline) {
                ...
                // Paginate Limit calculation // 
        $totalDocment=json_decode(json_encode($count_products))[0]->count;
        //^above  line was  $totalDocment= json_decode( json_encode(  $count_products ) ) ; $totalDocment[0]->count  
        // Convert skip to selected page 
        $totalpage=(int) ($totalDocment / $perpage);
        ...
        //  $this->products_search = $data_array  ;
        return $data_array;
    }

    public static function getallproducts ($_Getperpage) {
        ...
        if ( !isset($_REQUEST['page'])) {
            ...
            $request_page_number=(int) $_REQUEST['page'];
        }
        ...
        self::$resultpaginate = json_decode (json_encode($data_array))->paginate ;
